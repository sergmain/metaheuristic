 ...
 *
 * <p>One home dir, one H2 file DB and one SQLite file for the WHOLE JVM run, resolved in a static
 ...
 *
 * <p>Two file-backed stores are created here on purpose: {@link #DB_URL} is the MAIN datasource that
 * Liquibase migrates and {@code MetaStorageStubRepository} maps, and {@link #META_STORAGE_DB_PATH}
 * is the SQLite file the meta storage itself lives in. The test exists to prove those two coexist
...
public final class MetaStorageSharedItEnv {
    ...
    public static final String META_STORAGE_DB_PATH;
    ...
    static {
        try {
            ...
            META_STORAGE_DB_PATH = home.resolve("meta-storage/meta-storage.sqlite").toAbsolutePath().toString();
        }
        catch (Throwable e) {
