-- Meta storage schema for PostgreSQL
...
-- so no IDENTITY / AUTO_INCREMENT / SERIAL appears anywhere. That also sidesteps the int4 SERIAL
-- primary keys on high-churn tables that the Aurora audit flagged.
...
-- BODY is TEXT, so a 16k JSON body is TOASTed - compressed first, moved out of line only if it
-- still does not fit.
