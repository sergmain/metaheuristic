...
public class DownloadResourceActor extends AbstractTaskQueue<DownloadResourceTask> {
    ...
    public void fixedDelay() {
        ...
        while ((task = poll()) != null) {
            ...
            try {
                final String payloadRestUrl = task.mh.dispatcher..url + "/rest/v1/payload/resource/data";
                ...
                do {
                    try {
                        ...
                        Response response = HttpClientExecutor.getExecutor(
                                task.mh.dispatcher..url, task.mh.dispatcher..restUsername, task.mh.dispatcher..restPassword).execute(request);
                        ...
                        if (e.getStatusCode() == HttpServletResponse.SC_GONE) {
                            final String es = String.format("#810.035 Resource %s wasn't found on mh.dispatcher.. Task #%s is finished.", task.resourceId, task.getTaskId());
                            log.warn(es);
                            stationTaskService.markAsFinishedWithError(task.mh.dispatcher..url, task.getTaskId(), es);
