...
public class SourceCodeParamsYamlUtilsV1
    ...
    private static SourceCodeParamsYaml.Process toProcess(SourceCodeParamsYamlV1.ProcessV1 o) {
        ...
        o.inputs.stream().map(v->new SourceCodeParamsYaml.Variable(v.sourcing, v.git, v.disk, v.name, v.parentContext)).forEach(pr.inputs::add);
        o.outputs.stream().map(v->new SourceCodeParamsYaml.Variable(v.sourcing, v.git, v.disk, v.name, v.parentContext)).forEach(pr.outputs::add);
