...
public class MetaStorageService {
    ...
    public List<MetaStorageData.Record> select(Long companyId, String type, @Nullable List<String> recKeys) {
        if (recKeys==null || recKeys.isEmpty()) {
            return toRecords(metaStorageRepository.findAllByCompanyIdAndType(companyId, type));
        }
        ...
        for (int from = 0; from < recKeys.size(); from += MAX_KEYS_PER_QUERY) {
            ...
            result.addAll(toRecords(metaStorageRepository.findAllByCompanyIdAndTypeAndRecKeys(companyId, type, chunk)));
        }
        return result;
    }
    ...
    public int upsert(Long companyId, List<MetaStorageData.Record> records) {
        ...
        for (MetaStorageData.Record r : records) {
            final MetaStorage existing = metaStorageRepository.findByNaturalKey(companyId, r.type(), r.recKey());
            writes.add(new MetaStorageData.ResolvedWrite(existing==null ? null : existing.id, r));
        }
        return metaStorageTxService.upsert(companyId, writes, nextGeneration(companyId), System.currentTimeMillis());
    }
    ...
    public int deleteByNaturalKey(Long companyId, String type, String recKey) {
        final MetaStorage existing = metaStorageRepository.findByNaturalKey(companyId, type, recKey);
        if (existing==null) {
            return 0;
        }
        metaStorageTxService.deleteByIds(List.of(existing.id));
        return 1;
    }
    ...
    public List<String> listTypes(Long companyId) {
        return metaStorageRepository.findDistinctTypes(companyId);
    }
    ...
    public long generation(Long companyId, String type) {
        final Long max = metaStorageRepository.findMaxGen(companyId, type);
        return max==null ? 0L : max;
    }

    public long nextGeneration(Long companyId) {
        final Long max = metaStorageRepository.findMaxGenByCompanyId(companyId);
        return (max==null ? 0L : max) + 1L;
    }

    private static List<MetaStorageData.Record> toRecords(List<MetaStorage> rows) {
        final List<MetaStorageData.Record> result = new ArrayList<>(rows.size());
        for (MetaStorage row : rows) {
            if (row.body==null) {
