...
(nextval('mh_company_revision_id_seq'), 0,
 (select ID from MH_COMPANY where UNIQUE_ID=1), 1, 'Management company', '', false,
...
(
    ...
    KIND              VARCHAR(20) not null default 'MH'
);
...
-- ---------------------------------------------------------------------
-- MH_COMM_CHANNEL - a single-use token that mints ONE account bound to
-- one SERVICE.
--
-- A communication channel is an authenticated account bound to one REST
-- endpoint. Nothing here names any business concept: SERVICE_TAG is an
-- opaque label supplied by whichever module declared the service, and
-- GRANTED_ROLE is the role that module asked for. This mechanism is
-- generic infrastructure; what the outside party does with the
-- credential it receives is out of scope.
--
-- The COUNTERPARTY is whoever activates the token. INTENDED_FOR records
-- who the issuer MEANT to send it to, written at ISSUE time so a human
-- can later notice a mismatch. Nothing detects it automatically, and
-- recording it at activation would record whoever showed up - a
-- different fact.
--
-- Single-use is enforced by a COLUMN, not by cryptography:
-- ACTIVATED_ON IS NOT NULL means used, permanently. A nullable timestamp
-- makes "used" and "no longer valid" the same fact, written once.
--
-- EXPIRED_ON is an absolute epoch-millis deadline held in the row rather
-- than embedded in the token, so expiry needs no sweep job and no token
-- parsing to evaluate.
--
-- ISSUING and REVOKING are runtime operations on this table. Only the
-- SERVICE REGISTRY is configuration - services change when a deployment
-- changes, channels change while it runs, and a channel that needed a
-- redeploy to close would not be a control anyone could use.
-- ---------------------------------------------------------------------
create table MH_COMM_CHANNEL
(
    ID                    SERIAL PRIMARY KEY,
    VERSION               NUMERIC(10, 0) NOT NULL,
    -- mh_company.UNIQUE_ID of the company issuing the channel.
    COMPANY_ID            NUMERIC(10, 0) NOT NULL,
    -- Opaque service label from the registry; decides GRANTED_ROLE.
    SERVICE_TAG           varchar(50)    NOT NULL,
    -- The ROLE_* an activated account receives. Copied from the registry at
    -- ISSUE time so the grant is fixed when the operator decided it, not
    -- re-resolved later against a registry that may have changed.
    GRANTED_ROLE          varchar(100)   NOT NULL,
    -- High-entropy random, generated with SecureRandom. Unique installation-wide.
    TOKEN                 varchar(50)    NOT NULL,
    -- Free-text: who the issuer meant to give this to. Audit only, gates nothing.
    ...
    -- NOT NULL => already activated. This column IS the single-use guarantee.
    ACTIVATED_ON          bigint,
    -- The account minted on activation. NULL until then.
    ACCOUNT_ID            NUMERIC(10, 0),
    -- Withdrawn by an operator before activation, or revoked after it.
    IS_DELETED            BOOLEAN        NOT NULL DEFAULT FALSE
);

CREATE UNIQUE INDEX mh_comm_channel_token_unq_idx
    ON MH_COMM_CHANNEL (TOKEN);

CREATE INDEX mh_comm_channel_company_id_idx
    ON MH_COMM_CHANNEL (COMPANY_ID);

CREATE INDEX mh_comm_channel_account_id_idx
    ON MH_COMM_CHANNEL (ACCOUNT_ID);


-- LICENSE-038, local (offline signed-file) license manager.
--
-- mh_license_artifact holds ONE ROW PER INSTALLED LICENSE and every non-deleted row is live: an
-- installation holds a SET of licenses, not one. A trial cannot be extended (iat is fixed at
-- signing), so continuing one means installing a second license beside the first, and the
-- effective entitlement is the union of every currently-valid row.
--
-- TOKEN_HASH carries a unique index so re-installing a license you already have is a no-op rather
-- than a second row; without it the set fills with copies of one grant and the admin page becomes
-- unreadable. Removal flips IS_DELETED rather than deleting, so the audit trail survives.
--
-- CREATED_ON records when a license was installed on THIS dispatcher and is audit/display only.
-- It MUST NOT become an input to validity: expiry runs from the signed iat, and expiring relative
-- to a locally-recorded first-seen instant would reset every time the row was deleted.
--
-- mh_license_installation holds EXACTLY ONE ROW, written once at first boot: the installation
-- UUID that an installationId-bound license is checked against.

CREATE TABLE mh_license_artifact
(
    ID          SERIAL PRIMARY KEY,
    VERSION     NUMERIC(5, 0) NOT NULL,
    CREATED_ON  bigint        NOT NULL,
...
CREATE UNIQUE INDEX mh_license_artifact_token_hash_unq_idx
    ON MH_LICENSE_ARTIFACT (TOKEN_HASH);

CREATE TABLE mh_license_installation
(
    ID          SERIAL PRIMARY KEY,
    VERSION     NUMERIC(5, 0) NOT NULL,
    CREATED_ON  bigint        NOT NULL,
...
CREATE INDEX mh_meta_storage_company_type_gen_idx ON MH_META_STORAGE(COMPANY_ID, TYPE, GEN);

CREATE TABLE MH_META_STORAGE_REGISTRY
(
