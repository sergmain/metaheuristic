...
@NoArgsConstructor
public class Sect extends Composite implements XmlTag, HasProperty {

    private static final String NS = "wx";
    private static final String TAG_NAME = "sect";

    public Sect(CDNode... nodes) {
        super(nodes);
    }
    ...
    public void addPropertyElement(Class<? extends Property> propertyClass, PropertyElement propertyElement) {
        findProperty(propertyClass, propertyElement.getClass()).ifPresentOrElse(
                ...
                () -> findProperty(propertyClass).ifPresentOrElse(
                        ...
                        () -> {
                            if (propertyClass.isAssignableFrom(SectProp.class)) {
                                setProperty(new SectProp(propertyElement));
                            }
                        }
                ));
    }
